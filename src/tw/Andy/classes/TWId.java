...
public class TWId {
	private String id;
	// 從區域變數變為物件屬性
	public static String letters = "ABCDEFGHJKLMNPQRSTUVWXYWZIO";
...
//		StringBuffer  執行緒安全模式，較為安全，但較慢
//		StringBuilder 無執行緒安全模式，較為不安全，但較快
...
//		正規表示法 []代表範圍 {代表前面字元}重複數次
		if(id.matches("[A-Z][12][0-9]{8}")) {
			...
			int sum =a1*1 + a2*9 + n1*8 + n2*7 + n3*6 + n4*5 + n5*4 + n6*3
					+ n7*2 + n8*1 + n9*1;
